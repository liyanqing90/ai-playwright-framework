...
def load_project_context(project: str, env: str = "prod") -> ProjectContext:
    yaml = YamlHandler()
    env_config = yaml.load_yaml(Path("config/env_config.yaml"))
    project_cfg = (env_config.get("projects") or {}).get(project)
    if not project_cfg:
        raise ValueError(f"项目不存在: {project}")

    test_dir = Path(project_cfg.get("test_dir", f"test_data/{project}"))
...
def summarize_context(context: ProjectContext, *, max_items: int = 160) -> dict[str, Any]:
    return {
