...
from ai_playwright.utils.config import BaseInfo
...
def set_global_variables(
    ...
    variables = load_data.return_vars()
    if not variables:
        return
    variable_manager = VariableManager()
    current_temp_vars = variable_manager.list_variables("temp")
    if all(current_temp_vars.get(name) == value for name, value in variables.items()):
        return
    logger.info("当前测试环境: " + base_info.env)
    variable_manager.import_variables(variables, scope="temp", overwrite=True)
