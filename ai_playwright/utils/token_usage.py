    ...
    def record_model_io(
        ...
        *,
        ...
        with self._lock:
            ...
            data_policy = llm_data_policy(load_ai_config())
            payload = {
                "timestamp": _now().isoformat(timespec="seconds"),
                "run_id": self._active_run["run_id"],
                "operation": operation,
                "data_policy": data_policy,
                "request": redact_value(request_payload, policy=data_policy),
                "response": redact_value(response_payload, policy=data_policy),
                "error": redact_value(error, policy=data_policy),
            }
