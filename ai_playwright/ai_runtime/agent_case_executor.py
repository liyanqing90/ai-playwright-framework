...
from ai_playwright.ai_generation.pipeline import (
...
from ai_playwright.ai_runtime.ai_cache_store import ai_cache_path_from_config
from ai_playwright.ai_runtime.agent_plan_cache import (
    AgentCasePlanCache,
...
from ai_playwright.ai_runtime.cache_scope import (
...
from ai_playwright.ai_runtime.agent_text_utils import (
    ...
    _is_assertion_step,
    _is_external_url,
    _is_truncated_current_url_prefix,
    _normalize_assertion_text,
    _normalize_identifier_words,
    _normalize_text_list,
    _normalized_goal_text,
    _short_list,
    _step_action,
...
_AGENT_RUNTIME_ACTIONS = tuple(
    action for actions in _ACTION_CATEGORIES.values() for action in actions
)
...
class AgentCaseExecutor:
    ...
    def __init__(
        ...
        *,
        ...
        self.llm_data_policy = llm_data_policy(self.ai_config)
    ...
    def _try_run_compiled_agent_case(
        ...
        *,
        ...
        previous_assets = {
            ...
            "modules_cache": dict(
                getattr(self.step_executor, "modules_cache", {}) or {}
            ),
        }
        try:
            ...
            raise AssertionError(
                "Agent编译步骤执行失败: " f"case={case_name} | error={exc}"
            ) from exc
    ...
    def _refresh_dom_context(self, *, spec: dict[str, Any]) -> None:
        ...
        self.current_dom_context = build_dom_context(
            ...
            data_policy=self.llm_data_policy,
        )
    ...
    def _maybe_open_start_url(
        ...
        *,
        ...
        start_url = (
            ...
            or (
                ""
                if explicit_module_reference
                else _first_module_url(self.context.modules)
            )
    ...
    def _execute_step(self, step: dict[str, Any], *, spec: dict[str, Any]) -> None:
        self._apply_agent_step_defaults(step)
        action = _step_action(step).lower()
        before_dom_signature = self._step_dom_signature(step)
        before_url = str(getattr(self.page, "url", "") or "")
        previous_pages = _page_context_pages(self.page) if action == "click" else []
        new_page_probe_selector = (
            _resolve_project_selector(
                self.elements,
                str(step.get("selector") or step.get("target") or "").strip(),
            )
            if action == "click"
            else None
        )
        should_wait_for_new_page = bool(new_page_probe_selector) and (
            _click_may_open_new_page(
                self.page,
                new_page_probe_selector,
            )
        )
        new_page_url_hint = (
            _new_page_navigation_hint(self.page, new_page_probe_selector)
    ...
    def _apply_agent_step_defaults(self, step: dict[str, Any]) -> None:
        action = _step_action(step).lower()
        if action in {"", "goto", "wait", "use_module"}:
            return
        step.setdefault("timeout", self.agent_step_timeout_ms)
    ...
    def _post_action_failure_boundary(
        ...
        *,
        ...
        last_url_changed = (
            bool(last_action.get("url_changed"))
        ...
        if visible_errors and stalled_count >= 6:
            return "页面持续显示错误且多次动作未使URL变化: " + "; ".join(
                str(item) for item in visible_errors
            )
        return ""
    ...
    def _guard_decision(
        ...
        *,
        ...
        if decision.action == "use_module" and not spec.get("module_entry_allowed"):
            raise AgentDecisionRejected("use_module仅允许在用例显式声明项目模块时使用")
        ...
        if (
            ...
            and not guardrails.get("allow_payment_action", False)
    ...
    def _repair_risky_click_step(
        ...
        current_blob = _normalized_criterion_match_text(
            _element_primary_blob(current or {})
        )
        ...
        if not _risky_click_mismatch(
            current_blob=current_blob, intent_blob=intent_blob
        ):
            return
    ...
    def _realtime_agent_prompt_payload(
        ...
        *,
        ...
        completed_steps = (
            plan_status.get("completed") if isinstance(plan_status, dict) else []
        )
        remaining_steps = (
        ...
        current_step = (
            ...
            if isinstance(plan_status, dict)
                ...
                remaining_steps[0] if remaining_steps else "verify completion criteria"
            )
        )
        return {
...
def _runtime_plan_status(
    *,
    ...
    for step_text in plan:
        ...
        completed_count += 1
    current_index = (
        min(completed_count + 1, len(plan)) if completed_count < len(plan) else None
    )
    remaining_start = completed_count
    remaining_end = min(len(plan), remaining_start + max(1, int(lookahead or 1)))
    completed_steps = [
        normalize_model_text(item, limit=80) for item in plan[:completed_count]
    ]
    remaining_steps = [
    ...
    *,
    ...
    for text in _agent_natural_plan(spec):
        ...
        if not _natural_step_satisfied(
            ...
            if (
                last_satisfied
                and not _natural_step_target_observable(
                    step_text,
                    spec=spec,
                    dom_context=dom_context,
                    current_url=current_url,
...
def _natural_step_target_observable(
    ...
    *,
    ...
    if re.search(r"(?:输入|填写|填入|录入|fill|type|enter|input)", text, flags=re.I):
        ...
        return any(
            _dom_item_is_fillable(item) for item in _iter_dom_elements(dom_context)
        )
...
def _action_categories_for_prompt() -> dict[str, list[str]]:
    return {category: list(actions) for category, actions in _ACTION_CATEGORIES.items()}
...
def _intent_to_natural_steps(intent: str) -> list[str]:
    ...
    normalized = re.sub(
        r"(?:\s+then\s+|\s+and\s+then\s+)", "，", normalized, flags=re.I
    )
    parts: list[str] = []
...
def _auth_submit_history_satisfies_target(
    ...
    *,
    ...
    for item in _iter_history_items(history):
        ...
        before_url = str(step.get("_action_before_url") or "")
        after_url = str(step.get("_action_after_url") or item.get("url_after") or "")
        auth_route_changed = _url_looks_like_login(
            before_url
        ) and not _url_looks_like_login(after_url)
        if (
            not _history_step_has_observed_progress_in_history(history, item)
            and not auth_route_changed
        ):
            continue
...
def _fill_history_has_input_value(
    ...
    *,
    ...
    for item in _iter_history_items(history):
        ...
        if any(
            term and term in blob for term in terms
        ) or _fill_selector_matches_input_key(
            step.get("selector"),
            key,
        ):
            return True
    ...
    *,
    ...
    for key, candidate in _flatten_input_values(inputs):
        ...
        if (
            candidate_key
            and target_key
            and (candidate_key in target_key or target_key in candidate_key)
        ):
    ...
    for section, item in _iter_dom_elements_with_section(dom_context):
        ...
        if actions & {"click", "press"}:
            is_semantic_click_target = (
                ...
                or action_tag in {"a", "button", "input", "summary", "option"}
                or action_role
                in {
                    "button",
                    "link",
...
def _first_entry_module_name(
    *,
    ...
    return _module_name_from_entry_text(
        _first_instruction_clause(intent), modules=modules
    )
...
def _module_name_from_structured_step(step: dict[str, Any]) -> str:
    ...
    if action == "use_module":
        return str(
            step.get("module") or step.get("target") or step.get("name") or ""
        ).strip()
    return ""
...
def _structured_step_is_use_module(step: dict[str, Any]) -> bool:
    return (
        step.get("use_module") is not None
        or str(step.get("action") or "").strip().lower() == "use_module"
    )
...
def _text_declares_module_entry(text: str) -> bool:
    return bool(
        ...
        or re.search(
            r"\b(?:use|run|execute|call)\s+project\s+module\b", text, flags=re.I
        )
    ...
    *,
    ...
    if not data.get("selector") and isinstance(data.get("selectors"), list):
        ...
        if selector:
            ...
            logger.warning(
                "Agent模型将selector误写为selectors列表，已取首个有效selector"
            )
    allowed_fields = (
...
def _repair_click_target_from_spec(
    ...
    candidates: list[str] = []
    for text in texts:
        click_target = _click_instruction_target(text) or _implicit_interaction_target(
            text
        )
        if click_target:
...
def _history_step_semantic_values(step: dict[str, Any]) -> list[str]:
    ...
    for value in (
        ...
        if normalized:
            ...
            stripped = _normalized_criterion_match_text(_strip_action_words(normalized))
...
def _value_is_oa_login(value: Any) -> bool:
    ...
    return (
        bool(text)
        and "oa" in text
        and any(term in text for term in ("登录", "登陆", "login", "sign in"))
    )
...
def _criterion_evidence_terms(criterion: str) -> list[str]:
    ...
    terms.extend(re.findall(r"(?<![A-Za-z0-9_-])\d+(?![A-Za-z0-9_-])", text))
    terms.extend(
        match.strip(" ，。,.：:")
