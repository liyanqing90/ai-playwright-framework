...
from ai_playwright.ai_runtime.native_observe import build_selector_candidates
...
def compact_dom_candidates(
    ...
    *,
    ...
    The text model does not need geometry or raw class names, so those fields are
    dropped before LLM calls.
    """

    hint_terms = _terms(" ".join(_flatten_texts(hints or [])))
    effective_policy = _effective_data_policy(data_policy)
    scored: list[tuple[int, int, dict[str, Any]]] = []
    for position, candidate in enumerate(candidates):
        if not isinstance(candidate, dict):
            continue
        item = _compact_candidate(candidate, data_policy=effective_policy)
        if not item:
            continue
...
def build_dom_context(
    ...
    *,
    ...
    interactive = [
        _to_element(candidate, kind="interactive", data_policy=effective_policy)
        for candidate in compacted
        if _is_interactive(candidate)
    ]
    assertion_candidates = [
        _to_element(candidate, kind="assertion", data_policy=effective_policy)
        for candidate in compacted
        if _is_assertion_candidate(candidate)
    ]
    forms = [
        _to_form(candidate, data_policy=effective_policy)
        for candidate in compacted
    ...
    return {
        "meta": {
            "url": redact_value(url, policy=effective_policy),
            "title": redact_value(title, policy=effective_policy),
...
def build_locator_context(
    *,
    ...
    return {
        "locator_task": {
            "action": action,
            "target": redact_value(target, policy=effective_policy),
        },
...
def _rank_locator_elements(
    ...
    *,
    ...
    for index, element in enumerate(elements):
        ...
        score = 0
        if any(
            target_text and target_text in normalized_blob
    ...
    for item in history[-limit:]:
        ...
        if isinstance(page_errors, list) and page_errors:
            compact_step["_action_page_errors"] = [
                _trim_text(error, 120)
                for error in page_errors[:2]
                if str(error).strip()
            ]
        history_item = {
...
def _priority_hint_texts(values: list[Any]) -> list[str]:
    ...
    for value in values:
        ...
        if isinstance(value, dict):
            ...
            include_observation = isinstance(
                phase, str
            ) and _has_explicit_priority_action(phase)
...
def _effective_data_policy(data_policy: str | None) -> str:
    if data_policy:
        return normalize_llm_data_policy(data_policy)
    return llm_data_policy(load_ai_config())
    ...
    *,
    ...
    for key in _DOM_FIELDS:
        ...
        if key in _TEXT_LIMITS:
            if key in _REDACTABLE_DOM_FIELDS:
                value = redact_value(value, policy=data_policy)
...
def _to_element(
    ...
    *,
    ...
    item: dict[str, Any] = {
        ...
        "selector_candidates": _selector_candidates(candidate, data_policy=data_policy),
    }
    attributes = {
...
def _to_form(candidate: dict[str, Any], *, data_policy: str) -> dict[str, Any]:
    return {
...
def _selector_candidates(candidate: dict[str, Any], *, data_policy: str) -> list[str]:
    candidates = build_selector_candidates(candidate)
    if data_policy == LLM_DATA_POLICY_EXTERNAL:
        candidates = [item for item in candidates if item.selector_type != "text"]
    return [item.selector for item in candidates][:5]
...
def _selector_too_generic(selector: str) -> bool:
    ...
    return bool(
        re.fullmatch(
            r"(?:[a-z]+(?::nth-of-type\(\d+\))?\s*>\s*)*[a-z]+(?::nth-of-type\(\d+\))?",
            text,
        )
    )
...
def _is_unrequested_exit_candidate(text_blob: str, hint_terms: set[str]) -> bool:
    if not any(
        term in text_blob for term in ("退出登录", "退出", "登出", "注销", "logout")
    ):
        return False
    return not any(
