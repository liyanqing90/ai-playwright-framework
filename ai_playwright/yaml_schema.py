...
from ai_playwright.case_utils import TestCaseFile, yaml_handler
...
def _validate_action_step(
    ...
    spec = ACTION_SPEC_BY_NAME.get(action)
    if spec is None:
        return
    for field_name in spec.required:
        _require_present(context, path, raw_step, field_name)
    if spec.required_any:
        _require_any(context, path, raw_step, spec.required_any)
...
def _validate_selector_requirement(
    ...
    if isinstance(selector, dict):
        _validate_structured_selector(context, path, selector)
        return

    if not isinstance(selector, str):
        context.add_issue(path, "selector 必须是字符串或结构化 locator 对象")
        return
...
def _validate_structured_selector(
    context: ValidationContext,
    path: str,
    ...
    if unknown:
        context.add_issue(path, f"结构化 selector 包含未声明字段: {', '.join(unknown)}")
        return
    locator_keys = {
        "selector",
        "css",
    ...
    if not any(_has_required_value(selector, key) for key in locator_keys):
        context.add_issue(
            path,
    ...
    if "nth" in selector:
        _validate_timeout(context, f"{path}.nth", selector.get("nth"))
