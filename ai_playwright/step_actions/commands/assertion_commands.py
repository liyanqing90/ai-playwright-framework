...
from ai_playwright.step_actions.expression_evaluator import evaluate_math_expression
...
def _actual_checked(ui_helper, selector: str) -> bool:
    return _first_locator(ui_helper, selector).is_checked()
...
@CommandFactory.register(StepAction.ASSERT_CHECKED)
class AssertCheckedCommand(Command):
    ...
    def execute(
        ...
        ui_helper.assert_checked(selector=selector)
        _log_assertion_success(
            "assert_checked",
            selector=selector,
            expected=True,
            actual=_actual_checked(ui_helper, selector),
        )
