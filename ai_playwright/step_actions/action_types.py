...
class StepAction:
    ...
    SET_CHECKED = ["set_checked", "设置勾选"]
    TAP = ["tap", "轻触"]
    PRESS_SEQUENTIALLY = ["press_sequentially", "sequential_type", "顺序输入"]
    ...
    ASSERT_CHECKED = ["assert_checked", "验证选中"]
    ...
    GET_BOUNDING_BOX = ["get_bounding_box", "获取元素位置"]
    SELECT_TEXT = ["select_text", "选择文本"]
    ...
    GO_BACK = ["go_back", "后退"]
    GO_FORWARD = ["go_forward", "前进"]
    WAIT_FOR_URL = ["wait_for_url", "等待URL"]
    ...
    WAIT_FOR_FUNCTION = ["wait_for_function", "等待函数"]
    ADD_INIT_SCRIPT = ["add_init_script", "添加初始化脚本"]
    DISPATCH_EVENT = ["dispatch_event", "触发事件"]
    CAPTURE_SCREENSHOT = ["capture", "截图"]
    MANAGE_COOKIES = ["cookies", "Cookie操作"]
    SET_VIEWPORT_SIZE = ["set_viewport_size", "设置视口"]
    EMULATE_MEDIA = ["emulate_media", "模拟媒体"]
    ...
    WAIT_FOR_REQUEST = ["wait_for_request", "等待请求"]
    WAIT_FOR_RESPONSE = ["wait_for_response", "等待响应"]
    MOCK_ROUTE = ["mock_route", "模拟路由"]
    ABORT_ROUTE = ["abort_route", "拦截路由"]
    UNROUTE = ["unroute", "取消路由"]
    SET_OFFLINE = ["set_offline", "设置离线"]
    SET_EXTRA_HTTP_HEADERS = ["set_extra_http_headers", "设置请求头"]
    GRANT_PERMISSIONS = ["grant_permissions", "授予权限"]
    CLEAR_PERMISSIONS = ["clear_permissions", "清除权限"]
    SAVE_STORAGE_STATE = ["save_storage_state", "保存存储状态"]
