...
在进入浏览器之前，AI Playwright 还会检查候选用例是否至少包含一项有效断言。下面是[候选验证与正式写入实现](https://github.com/liyanqing90/ai-playwright-framework/blob/5e90d7db1e149de20f06b047811c1497df79002d/ai_playwright/ai_generation/case_generator.py)中压缩出的核心逻辑。为了便于阅读，省略了类型保护和完整错误信息，但保留了 `data_name`、上下文模块合并与递归断言检查这些关键语义。
...
*图 1｜候选结果经过断言、Schema、真实执行和正式写入后的二次验证，才进入代码评审与 CI。微信公众号版本直接使用已导出的流程图图片。*
...
[把自然语言目标绑定到稳定元素键的修正](https://github.com/liyanqing90/ai-playwright-framework/commit/328222e336f69e7a5276b802f7e902577c40b777)解决的不是“模型能不能找到按钮”，而是模型找到之后，能否把结果沉淀成后续测试可以复用和审查的资产。

[复用浏览器已经验证过的 Selector 资产](https://github.com/liyanqing90/ai-playwright-framework/commit/98d17f79d582b56921ab2ac4c2f0ba3aedd877b9)处理了相邻问题：即使本轮候选最终失败，已经得到浏览器验证的 Selector 修复也不应该全部丢弃。系统会记录这些更新，并在下一轮修复时复用已经获得的证据。

## 失败需要留下可继续工作的证据

真实页面验证失败时，AI Playwright 会保留本次生成目录、候选文件、pytest 输出、模型 Payload 和错误信息。成功时这些临时产物会清理；失败时则保留下来，供下一轮定位和修复。
...
完整实现和复现步骤已经整理到公开仓库。首次阅读不需要理解仓库目录，可以直接从下面的入口进入：

- [查看为什么生成命令强制执行真实验证](https://github.com/liyanqing90/ai-playwright-framework/blob/5e90d7db1e149de20f06b047811c1497df79002d/ai_playwright/cli/generate_case.py)
- [查看候选验证、正式写入与失败产物处理](https://github.com/liyanqing90/ai-playwright-framework/blob/5e90d7db1e149de20f06b047811c1497df79002d/ai_playwright/ai_generation/case_generator.py)
- [按步骤运行本文的完整示例](https://github.com/liyanqing90/ai-playwright-framework/blob/main/docs/articles/ai-generated-tests-verification-first/reproduce.md)
- [查看项目实际使用的完整 CI 质量门](https://github.com/liyanqing90/ai-playwright-framework/blob/5e90d7db1e149de20f06b047811c1497df79002d/.github/workflows/ci.yml)

只想复现实验时，直接打开“按步骤运行本文的完整示例”即可。
